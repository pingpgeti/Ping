<template>
  <nav class="navbar" :class="{'navbar--active': isActive}">
    <div class="navbar__wrapper" :class="{'navbar__wrapper--moved' : !isLogoVisible}">
      <div @click="isActive = !isActive" class="burger navbar__burger" :class="{'burger--active': isActive}">
        <span/>
        <span/>
        <span/>
      </div>
      <svg
          class="navbar__logo"
          :class="{'navbar__logo--hidden' : !isLogoVisible}"
          width="99"
          height="28"
          viewBox="0 0 99 28"
          fill="none"
